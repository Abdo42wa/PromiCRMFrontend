export const productMaterialsReducer = (state = { materials: [], order_materials: [], modified_order_materials: [] }, action) => {
    switch (action.type) {
        ...
        case 'MATERIALS_ORDER_DELETE_SUCCESS':
            ...
            const m_index = state.modified_order_materials.findIndex(x => x.id === action.payload)
            if (m_index !== -1) {
                const new_m_order_materials = state.modified_order_materials.filter(x => x.id !== action.payload)
                return { ...state, loading: false, order_materials: order_materials_data, modified_order_materials: new_m_order_materials }
            } else {
