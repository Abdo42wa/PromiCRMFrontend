...
export const getNonStandartWorks = (callback) => async(dispatch,getState)=>{
    try{
        ...
        const response = await axios.get(`/api/NonStandardWorks`, {headers: {Authorization: `Bearer ${token}`}});
        dispatch({
...
export const createNonStandartWork = (postObj, callback) => async(dispatch,getState) =>{
    try{
        ...
        const token = getState().usersReducer.currentUser;
        const response = await axios.post(`/api/NonStandardWorks`,postObj, {headers: {Authorization: `Bearer ${token}`}});
        dispatch({
...
export const updateNonStandartWork = (postObj,reducerObj,callback) => async(dispatch,getState) =>{
    try{
        ...
        // get token from usersReducer
        const token = getState().usersReducer.currentUser;
        const response = await axios.put(`/api/NonStandardWorks/${reducerObj.id}`,postObj, {headers: {Authorization: `Bearer ${token}`}});
        dispatch({
